...
if act="getremark" then 
	firstid=0
	set rsfirst=dbconn.execute("select top 1 maxid from gdgs_remark  order by id desc")
	if not rsfirst.eof  then firstid=rsfirst(0)
	url="http://gdgs.chinaxinge.com/scan_data.asp?act=getRemark&id="&firstid
	wd=getpage(url)
	if wd<>"" then
		wd=replace(wd,"","")
		wd=replace(wd,"%","")
		...
		dbconn.execute("insert into gdgs_remark(maxid,result,kwd) values('"&maxid&"','"&result&"','"&wd&"')")		
		response.Write("{""error_code"":200,""maxid"":"""&maxid&""",""icount"":"""&icount&""",""wd"":"""&wd&""",""result"":"""&result&"""}")
	else
...
if act="getremarkall" then
	...
	set rsfirst=dbconn.execute("select top 1 maxid from gdgs_remark  order by id desc")
	if not rsfirst.eof  then firstid=rsfirst(0)
	url="http://gdgs.chinaxinge.com/scan_data.asp?act=getRemarkAll&id="&firstid
	wd=getpage(url)
	dbconn.close()
	response.Write("{""error_code"":200,""result"":"""&wd&"""}")
end if
