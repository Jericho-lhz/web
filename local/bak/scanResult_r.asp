...
function getResult(){
	if(id!=''){
		$.ajax({
			...
			success: function(res){
				if(res.error_code==200){
					var kwd=res.kwd.replaceAll('}{','<br>').replaceAll('{','').replaceAll('}','')
					var result=res.result
					result=result.replaceAll('-','').replaceAll('&','|').replace(/[a-zA-Z]+/g,"").replace(/[0-9]+/g,"")
					var arr=result.split('|')
					$.each(arr,function(i){
						console.log(arr[i])
						if(i>0){
							kwd=kwd.replaceAll(arr[i],'<span style="color:#fff;background:#f70">'+arr[i]+'</span>')
						}
					})
					$('.main-wd').html(res.result+'<br><br>'+kwd)
